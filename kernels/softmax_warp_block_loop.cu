...
__global__ void softmax_max_kernel(const float* data, float* max_out, int batch_size, int n) {

  int tx = threadIdx.x;
  float val = 0.0f;
  unsigned mask = __activemask(); 
  __shared__ float seme[BLOCKSIZE / 32]; 
  int lane_id = tx & 31, warp_id = tx >> 5, ibatch = blockIdx.x;
  float warp_max_val_acc = -MAXFLOAT;

  for (int istart = 0; istart < n; istart += BLOCKSIZE) {

    if ((istart + tx) < n){
      val = data[istart + tx + n * ibatch];
    }
    else{
      val = 0.0f;
    }

    float down_val = reduce_max_warp_down(mask, val); 
    if (lane_id == 0){
      seme[warp_id] = down_val;
    }
    __syncthreads();
    ...
    if (warp_id == 0){
      ...
      if (lane_id == 0) {
        if (warp_max_val > warp_max_val_acc)
        {
          warp_max_val_acc = warp_max_val;
        }
      }
    }
  ...
  {
    max_out[ibatch] = warp_max_val_acc;
  }
...
__global__ void softmax_block_loop_kernel(const float* data, float* max_val, float* out, int batch_size, int n) {
  ...
  int tx = threadIdx.x;
  float val = 0.0f;
  float val_copy[LOOPSIZE];
  unsigned mask = __activemask(); 
  __shared__ float seme[BLOCKSIZE / 32]; 
  int lane_id = tx & 31, warp_id = tx >> 5, ibatch = blockIdx.x;
  float warp_sum_val_acc = 0.0f;

  for (int istart = 0; istart < n; istart += BLOCKSIZE) {

    if ((istart + tx) < n){
      val = expf(data[istart + tx + n * ibatch] - max_val[ibatch]);
    }
    else{
      val = 0.0f;
    }

    val_copy[istart / BLOCKSIZE] = val;
    ...
    if (warp_id == 0){
      ...
      if (lane_id < (BLOCKSIZE / 32)) {
        seme[lane_id] = warp_sum_val;
      }

    }
    __syncthreads();
    warp_sum_val_acc += seme[warp_id];
  ...
  for (int istart = 0; istart < n; istart += BLOCKSIZE){
    if ((istart + tx) < n){
      out[istart + tx + n * ibatch] = val_copy[istart / BLOCKSIZE] / warp_sum_val_acc;
    }
  }
...
cudaError_t launch_softmax_block_loop(const float* input, float* max_val, float* output, SoftmaxShape shape, cudaStream_t stream) {
  ...
  softmax_max_kernel<block><<<grid, block, 0, stream>>>(
    input, max_val, shape.rows, shape.cols
  );
