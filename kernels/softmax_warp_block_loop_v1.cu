...
__global__ void softmax_block_loop_kernel(const float* data, float* max_val, float* out, int batch_size, int n) {
    ...
    float warp_sum_val_acc = 0.0f, warp_max_val_acc = -MAXFLOAT;
    ...
    {
        max_val_shared = warp_max_val_acc;
    }

    __syncthreads();
    warp_max_val_acc = max_val_shared;


    for (int istart = 0; istart < n; istart += BLOCKSIZE) {

        val = expf(ori_val[istart / BLOCKSIZE] - warp_max_val_acc);
...
cudaError_t launch_softmax_block_loop_v1(const float* input, float* max_val, float* output, SoftmaxShape shape, cudaStream_t stream) {
